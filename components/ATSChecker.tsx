...
const ATSChecker: React.FC<{ initialResumeText?: string }> = ({ initialResumeText }) => {
  ...
  const getSeverityColor = (severity: ATSFeedback['severity']) => {
    switch (severity.toLowerCase()) {
      ...
      case 'low': return 'bg-blue-100 dark:bg-blue-900/30 border-blue-500';
  ...
  return (
    <div className="max-w-4xl mx-auto">
      <div className="bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md transition-colors duration-200">
        <h2 className="text-2xl font-bold mb-4 text-gray-800 dark:text-white">ATS Score Checker</h2>
        <p className="text-gray-600 dark:text-gray-300 mb-6">Upload your resume to analyze its compatibility with Applicant Tracking Systems (ATS).</p>
        ...
        {isUsingBuilderResume ? (
            ...
            <div className="flex items-center justify-center w-full">
                <label htmlFor="dropzone-file" className="flex flex-col items-center justify-center w-full h-64 border-2 border-gray-300 dark:border-gray-600 border-dashed rounded-lg cursor-pointer bg-gray-50 dark:bg-gray-700 hover:bg-gray-100 dark:hover:bg-gray-600 transition-colors">
                    <div className="flex flex-col items-center justify-center pt-5 pb-6 text-center px-4">
                        ...
                        {fileName ? (
                          ...
                          <>
                            ...
                            <p className="text-xs text-gray-500 dark:text-gray-400">Word, PDF, or TXT files</p>
                          </>
                        )}
                    </div>
        ...
        <button
          ...
          disabled={isLoading}
          className="mt-4 w-full bg-blue-600 text-white px-6 py-3 rounded-md font-semibold hover:bg-blue-700 disabled:bg-blue-300 dark:disabled:bg-blue-800 transition-colors flex items-center justify-center"
        >
      ...
      {analysisResult && (
        <div className="mt-8 bg-white dark:bg-gray-800 p-6 rounded-lg shadow-md transition-colors duration-200">
          <h3 className="text-2xl font-bold mb-6 text-center text-gray-800 dark:text-white">Analysis Report</h3>
          <div className="grid grid-cols-1 md:grid-cols-2 gap-8 items-center">
            <div className="flex justify-center">
              <ScoreGauge score={analysisResult.overallScore} />
            </div>
            <div>
              <h4 className="font-semibold text-lg mb-3 text-gray-800 dark:text-white">Score Breakdown</h4>
          ...
          <div className="mt-8">
            <h4 className="font-semibold text-xl mb-4 text-gray-800 dark:text-white">Actionable Feedback</h4>
            <div className="space-y-4">
              {analysisResult.feedback.map((item, index) => (
                <div key={index} className={`p-4 border-l-4 rounded ${getSeverityColor(item.severity)}`}>
                  ...
                  <p className="text-gray-600 dark:text-gray-300">{item.suggestion}</p>
                </div>
              ))}
            </div>
          </div>
